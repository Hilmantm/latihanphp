<?php
  ...
  function registrasi($data) {
    ...
    $password = htmlspecialchars(mysqli_real_escape_string($connection, $data["password"]));
    $passwordConfirm = htmlspecialchars(mysqli_real_escape_string($connection, $data["passwordConfirm"]));

    // check if username is exist
    $check = mysqli_query($connection, "SELECT * FROM users WHERE username='$username'");
    if( mysqli_fetch_assoc($check) ) {
      echo "<script>alert('username sudah tersedia, gunakan username lain')</script>";
      return false;
    }
